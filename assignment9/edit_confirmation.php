<?php
	...
	if ( !isset($_POST['title']) || empty($_POST['title']) ) {
		...
		$statement = $mysqli->prepare("UPDATE dvd_titles SET title = ?, release_date = ?, award = ?, genre_id = ?, label_id = ?, rating_id = ?, format_id = ?, sound_id = ?  WHERE dvd_title_id = ?");

		$statement->bind_param("sssiiiiii", $_POST["title"], $release_date, $award, $genre, $label, $rating, $format, $sound, $_GET["dvd_title_id"]);
...
<body>
	...
	<div class="container">
		...
		<div class="row mt-4 mb-4">
			<div class="col-12">
				<a href="details.php?dvd_title_id=<?php echo $_GET['dvd_title_id']; ?>" role="button" class="btn btn-primary">Back to Details</a>
			</div> <!-- .col -->
		</div> <!-- .row -->
	</div> <!-- .container -->
</body>
